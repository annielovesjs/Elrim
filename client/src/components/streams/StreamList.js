                ...
                <React.Fragment>
                    <Link to={`/streams/show/${stream._id}`} className={streamsStyles.streamItem} key={stream._id}>
                        <div className={streamsStyles.streamPicFrame}>
                            <img style={{width: "100%", height: "100%"}} alt="stream preview" className="ui tiny image middle aligned streamPic" src={stream.image ? `data:image/png;base64,${stream.image.url}`: this.props.userImageUrl}></img>
                        </div>
